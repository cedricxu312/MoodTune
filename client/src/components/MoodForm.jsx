...
export default function MoodForm({ onMoodFetched, setShowCreateButton, showCreateButton, suggestedMood }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!mood.trim()) return setError('Please enter a mood');
    setLoading(true);
    setError(null);
    setPlaylistCreated(false);  // Reset playlist created state
    setPlaylistUrl(null); // Reset playlist URL
  ...
  const handleCreatePlaylist = () => {
    const popup = window.open('http://127.0.0.1:3001/create-playlist', 'spotify-auth', 'width=500,height=600');
  ...
  return (
    ...
    <button
      ...
      style={{
        ...styles.primaryBtn,  // Using the same base style as Get Tracks button
    ...
    >
      {loading ? 'Analyzing mood and fetching songs... It may take a while' : 'Get Tracks'}
    </button>
  </div>
